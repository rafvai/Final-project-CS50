...
bool vote(int rank, string name, int ranks[])
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        //compare string name with candidates' name
        if (strcasecmp(name,candidates[i]) == 0)
        {
            //update the array rank for each position (rank)
            ranks[rank] = i;
...
void record_preferences(int ranks[])
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        //loop to compare the next candidate with i
...
void add_pairs(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        //loop through the candidates from the last to compare with i
        for (int j = candidate_count - 1; j >= 0; j--)
        {
            //look if there is a winner
            if (preferences[i][j] > preferences[j][i])
            {
                //update the array with the number of the candidate winner
                pairs[pair_count].winner = i;

                //update the array with the number j of the candidate that lose
